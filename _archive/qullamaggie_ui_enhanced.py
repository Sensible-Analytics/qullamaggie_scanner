...
class QullamaggieUI:
    ...
    def __init__(self, root):
        ...
        self.current_scan_total = {}
        self.current_scan_current = {}
    ...
    def start_scan(self):
        ...
        for scan_id in ['1M', '3M', '6M']:
            ...
            for item in tree.get_children():
                tree.delete(item)
            # Reset progress
            progress_bar = getattr(self, f"progress_bar_{scan_id}")
            progress_bar['value'] = 0
            progress_label = getattr(self, f"progress_label_{scan_id}")
            progress_label.config(text="Waiting to start...")
    ...
    def run_scan(self, config):
        ...
        try:
            ...
            self.scanner.set_progress_callback(lambda data: self.progress_queue.put(data))
            
            # Connect to TWS
            self.progress_queue.put({'type': 'status', 'message': 'Connecting to TWS...'})
            success, msg = self.scanner.connect()
            
            if not success:
                self.progress_queue.put({'type': 'error', 'message': f"Failed to connect: {msg}"})
                return
            
            self.progress_queue.put({'type': 'log', 'message': "Connected to TWS", 'level': 'SUCCESS'})
            ...
            self.progress_queue.put({'type': 'final_results', 'results': results})
            
            # Auto-export if enabled
            if self.auto_export_var.get() and results:
                self.progress_queue.put({'type': 'auto_export', 'results': results})
            
            self.progress_queue.put({'type': 'log', 'message': f"Scan complete! Found {len(results)} setups", 'level': 'SUCCESS'})
            
        except Exception as e:
            self.progress_queue.put({'type': 'error', 'message': f"Scan error: {str(e)}"})
    ...
    def check_progress_queue(self):
        ...
        try:
            while True:
                ...
                if msg_type == 'status':
                    self.status_label.config(text=data.get('message', ''))
                    ...
                    self.current_scan_current[scan_id] = 0
                    self.current_scan_total[scan_id] = 0
                    
                    progress_label = getattr(self, f"progress_label_{scan_id}")
                    progress_label.config(text=f"Starting {scan_name} scan...")
                    self.log(f"Starting {scan_name} scan", "INFO")
                    ...
                    self.current_scan_current[scan_id] = current
                    self.current_scan_total[scan_id] = total
                    ...
                    # Update label
                    progress_label = getattr(self, f"progress_label_{scan_id}")
                    progress_label.config(text=f"Analyzing {symbol} ({current}/{total})")
                    ...
                    adr = data.get('adr', 0)
                    self.log(f"{scan_id}: Found {symbol} - Score: {score}, ADR: {adr:.1f}%", "SUCCESS")
                    ...
                    scan_id = data.get('scan_id')
                    message = data.get('message', '')
                    self.log(f"{scan_id} scan error: {message}", "ERROR")
                
                elif msg_type == 'final_results':
                    results = data.get('results', [])
                    self.update_final_results(results)
    ...
    def update_scan_results(self, scan_id, results):
        ...
        # Sort by score
        results_sorted = sorted(results, key=lambda x: x['qulla_score'], reverse=True)
        ...
        progress_label = getattr(self, f"progress_label_{scan_id}")
        progress_label.config(text=f"Complete - {len(results)} setups found")
